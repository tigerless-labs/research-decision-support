...
对设计上有争议的点跑决策模型（驱动力→选项→权衡→取舍），开放态为决策工作表，定了坍缩成一条 ADR。**ADR 只增不删**，旧决策标记被取代而非抹除。决策锚定到 [design/](../downstream-design/index.md) 的元素。方法由 `research-decision-support` skill 承载。
