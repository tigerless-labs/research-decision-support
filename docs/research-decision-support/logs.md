...
- 2026-07-10 · output 同步（modules/canvas + architecture） · 装配 · 旧：canvas.md 记"默认 canvas/style.css"、残差含默认暗色 palette，architecture canvas/ 树含 style.css → 新：canvas/ 不放任何 CSS、默认改指 pin-and-paper 同格同校验、暗色残差随之划掉；SKILL.md 与 build_canvas.py（DEFAULT_STYLE=pin-and-paper）同步
