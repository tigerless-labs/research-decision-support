...
- 2026-07-13 · 最小渲染契约成文＋静默失败改报错（protocol.md + workspace/check_workspace） · 装配（人："尽量减少固定规则，总结显示必需的最小 schemas"） · 旧：渲染契约只隐含在解析器行为里；未知顶层目录整卡静默消失、未闭合 frontmatter 漏入正文、块列表 tags 静默当无 tag → 新：protocol.md 增 Rendering contract 一节（引擎底线三条＋兜底承诺，与方法论约束分层）；tags 支持 YAML 块列表（规则变适配）；校验器新增未知目录/坏 frontmatter 两查；73 测试全绿
