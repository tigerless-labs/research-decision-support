...
接口分界，见 [architecture](../architecture.md)）：
...
- 模板与风格彻底分离：`canvas/` 只有结构（spec + template，留 CSS 注入槽），
  **不放任何 CSS**，全部 CSS 归风格包；**换风格只换 CSS**（`--css` 传风格文件），
...
- 风格 CSS **预编译入库、格式统一**：每个 style 目录下一份 `canvas.css`
  ...
  直接指过去，零等待；校验器查 canonical token 明暗双份齐全、禁外链。构建
  默认风格＝`styles/pin-and-paper/canvas.css`（画布的原生观感），与其余风格
  完全同格、同校验。
