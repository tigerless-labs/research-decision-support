...
## 追加：方案 C 钩子 + swiss-modern 打样（2026-07-13）

上一轮只动卡片层，观感差异不足（用户核验）。按方案 C 给模板开风格钩子（chrome 空壳、
wtitle 序号槽、边形状 CSS 变量、换肤槽具名），并以 swiss-modern 一个风格整篇重写 CSS
（背景网格、标题、标签、边、工具栏 chrome）对齐参考 artifact 打样验收；其余 7 风格靠
钩子默认值保持原观感，打样通过后再逐风格铺开。单元顺序不变：docs → tests（模板钩子
契约锁）→ template + swiss CSS → 构建核验 → artifact 重发。
