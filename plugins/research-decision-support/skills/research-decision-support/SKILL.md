...
description: Decision support for research — weave what you read (papers, repos, posts, reports) into a provenance-linked design you can defend. Five structural layers (sources → synthesis → ideas → design → decisions), maintained in any order, each with a fixed card format; the agent files evidence and coaches the method, the judgment stays yours. Use when doing a literature review, evaluating vendors/tools, due diligence, competitive analysis, or turning reading into an architecture or method design — triggers like "file these papers", "compare these directions", "assemble these into a design", "decide this contested point", 整理这批论文、做个方向对比、把这些想法组装成设计、这个设计点怎么决策.
license: MIT
...
# research-decision-support — reading → defensible design

When triggered, enter **synthesis mode**. The five layers are **structure, not stages** — move
across them at the pace real work happens (sketch a design after the first batch of sources,
read more, revise an idea, reopen a decision), in any order. The skill never imposes a
sequence; it does exactly one job: **file every block the user produces into the right layer,
keep it atomic, keep the links unbroken.** Layers relate by provenance (design builds on
ideas, ideas cite sources, decisions act on design), not by time.
...
- **Three voices never share a card**: literature note (source says) ≠ permanent note (I say)
  ≠ design (we build). See [note-types](references/note-types.md).
...
then start. Bottom-up (sources → ideas → design) and top-down (sketch design first, backfill)
are both fine.

## Start light — layers earn their place

Don't impose all five layers on a young workspace. Start with **sources + ideas** only.
...
- **design** — when adopted ideas start fitting together into something buildable.
...
- **An adopted idea sits unused** (no design references it) → point it out when timely:
  assemble it into the design, or downgrade/cut it.
...
## The five layers (maintain in any order)
...
### design — `design/`
Weave adopted ideas into a design, two file kinds
([design-provenance](templates/design-provenance.md)):
- **One spine**: principles + pipeline + global invariants, with an architecture diagram;
  every element gets a stable ID.
- **One file per step/module**: its behavior boundary, interface contract, acceptance criteria;
  global invariants live only in the spine.
- `index.md` is pointers only. Provenance chains throughout: **design element → ideas →
  source cards**.

### decisions — `decisions/`
Contested design points get a worksheet ([decision-worksheet](templates/decision-worksheet.md)),
...
append-only) that acts back on the design (`affects` ↔ provenance). Decisions and design
cross-reference and co-evolve — no forced ordering.
...
workbench (overview / read / compare / ideas / design / decisions / map + card reader):
