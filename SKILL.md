...
- 每次写入后跑校验器：`python3 tools/check_doc_links.py docs` 和 `python3 tools/check_research_loom.py docs/research-loom`。
...
需要给人看/给自己读时渲染工作台：`python3 tools/build_loom_site.py <workspace> -o <outdir> --title <名字>`
生成 overview / read / compare / map 四页自包含 HTML。read 页支持读中打标（状态/方向/一句话判断，
浏览器草稿），导出的补丁交回 agent 落进卡片——markdown 永远是唯一真身。收到这类补丁时逐条写回
对应卡的 frontmatter 与正文。
